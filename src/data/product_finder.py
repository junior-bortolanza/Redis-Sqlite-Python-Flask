...
class ProductFinder:
    ...
    def __find_in_cache(self, product_name: str) -> tuple:
        products_infos = self.__redis_repo.get_key(product_name) # celular: "199.99, 1"
        if products_infos:
            products_infos_list = products_infos.split(",") # price, quantity -> [price, quantity]
            return (0, product_name, products_infos_list[0], products_infos_list[1])
    ...
    def __find_in_sql(self, product_name: str) -> tuple:
        ...
        if not product: 
            raise Exception("Produto nao foi encontrado")
